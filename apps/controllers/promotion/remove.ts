...
export const removePromotion = async (req: any, res: Response): Promise<any> => {
  const requestQuery = req.query as PromotionAttributes
  ...
  try {
    const result = await PromotionsModel.findOne({
      where: {
        deleted: { [Op.eq]: 0 },
        promotionId: { [Op.eq]: requestQuery.promotionId }
      }
    })
